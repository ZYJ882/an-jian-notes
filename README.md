...
| 全屏详情页 | 单击列表笔记后进入完整详情页面；无需点击编辑按钮，直接单击标题或正文即可在当前位置进入原地编辑并唤起软键盘；页面结构和滚动位置保持不变，完成后可继续预览或保存。 |
| 链接安全交互 | 链接不会因点击自动跳转。在编辑正文时选中完整 URL，会出现应用自定义的浮动“跳转链接”操作条；点击后仍需确认才会使用系统浏览器打开。 |
| 快速响应 | 搜索采用 80ms 极短防抖；列表和详情页面使用 220ms 缩放淡入与 120ms 淡出过渡，选中链接操作条采用轻量弹出动画；数据库容器由应用级懒初始化管理。 |
