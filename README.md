...
| Markdown 编辑与预览 | 编辑器内提供自动识别、Markdown、纯文本三种格式模式；预览会按照已保存的笔记格式显示，不会将纯文本误标记为 Markdown；支持标题、粗体、斜体、删除线、行内代码、列表、引用与分隔线的轻量预览。 |
...
| 收藏夹 | 首次打开自动创建“默认收藏夹”；顶部横向切换收藏夹；在当前收藏夹中新建、导入文件或导入剪切板文本时，笔记会自动归属当前收藏夹；详情页可将笔记移动到其他收藏夹。 |
| 全屏详情页 | 单击列表笔记后进入完整详情页面；所有既有笔记默认预览，双击详情内容区域进入编辑；编辑与预览采用一致的标题、元信息和正文宽度；Markdown 仍可通过顶部“编辑 / 预览”按钮切换。 |
