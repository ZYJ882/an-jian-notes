...
| 加号创建菜单 | 点击右下角加号，可手写新笔记、导入文件或从剪切板导入；导入文件会直接调起系统文件管理器，并标注支持 `.md`、`.markdown`、`.txt` 和 UTF-8 文本。 |
| 自动格式识别 | 手写和剪切板导入内容会根据标题、列表、引用、强调、代码等常见语法自动判定 Markdown 或纯文本；可在详情页中手动覆盖。 |
| 全屏详情页 | 点击笔记后进入完整详情页面；Markdown 默认预览，点击“编辑 / 预览”切换；纯文本直接编辑。 |
